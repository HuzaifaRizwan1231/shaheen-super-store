...
if (isset($_GET['add_to_cart'])) {
    if (!isset($_SESSION["user_id"])) {
        echo "<script>triggerToast( 'Please Log in first to add items to cart', 'danger')</script>";
    } else {
        ...
        if ($rows == 1) {
            echo "<script>triggerToast( 'Product is already added to Cart', 'warning')</script>";
        } else {
            $query = "INSERT INTO orders(user_id,product_id,order_status) VALUES ($user_id,$product_id,'Pending')";
            $result = mysqli_query($con, $query);
            echo "<script>triggerToast( 'Added to Cart', 'success')</script>";
        }
    }
}
